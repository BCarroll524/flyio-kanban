...
- Create two apps on Fly, one for staging and one for production:
  ...
  ```sh
  fly apps create flyio-example-6d10
  fly apps create flyio-example-6d10-staging
  ```
...
- Add a `SESSION_SECRET` to your fly app secrets, to do this you can run the following commands:
  ...
  ```sh
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app flyio-example-6d10
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app flyio-example-6d10-staging
  ```

  > **Note:** When creating the staging secret, you may get a warning from the Fly CLI that looks like this:
  >
  > ```
  > WARN app flag 'flyio-example-6d10-staging' does not match app name in config file 'flyio-example-6d10'
  > ```
  >
...
- Create a database for both your staging and production environments. Run the following:
  ...
  ```sh
  fly postgres create --name flyio-example-6d10-db
  fly postgres attach --app flyio-example-6d10 flyio-example-6d10-db

  fly postgres create --name flyio-example-6d10-staging-db
  fly postgres attach --app flyio-example-6d10-staging flyio-example-6d10-staging-db
  ```
